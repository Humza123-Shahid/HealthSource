...
const AddPrescription = () => {
   const [showToast,setShowToast]=useState(false)
     ...
     const [credentials,setCredentials] =useState({consultationId:null,doctorId:null,patientId:null,notes:"",medicine:null,dosage:"",frequency:"",duration:"",instructions:""})
         const [ issuedDate, setIssuedDate] = useState(undefined);
         const [ issuedDate2, setIssuedDate2] = useState(undefined);
  ...
  const handleChange4 = (selectedOption) => {
        if(selectedOption=="" )
        {
            setCredentials({...credentials,'medicine':null})
        }
        else
        {
          setCredentials({...credentials,'medicine':selectedOption.value})
        }
  };
...
const filterOption = (option, inputValue) => {
  ...
  const addPrescriptions=async (e)=>{
         e.preventDefault();
        const {consultationId,doctorId,patientId,notes,medicine,dosage,frequency,duration,instructions}=credentials

         
          const user=await addPrescription(consultationId,doctorId,patientId,issuedDate2,notes,medicine,dosage,frequency,duration,instructions)
          console.log(user)
          if(user.success)
          {
            ...
            console.log("abc");
            setShowToast(true);
            setMsg("Prescription added successfully")
            setType("success")
            setTimeout(()=>{
              setShowToast(false)
            },1500)
...
useEffect(() => {
  ...
  return (
    ...
    <form onSubmit={addPrescriptions}>
    <div className='mx-0' style={{display:'flex'}}>
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="consultationId" className="form-label">Consultation</label>
            {/* <select id="consultationId" className="form-control " name="consultationId" onChange={onChange}>
            ...
            return(
             ...
             <Select id="consultationId" options={options3} filterOption={filterOption} onChange={handleChange3} name="consultationId" placeholder="Select Consultation" />
        </div>
     ...
     <hr />
      ...
      <div className='mx-0' style={{display:'flex'}}>
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            {/* <select id="medicine" className="form-control " name="medicine" onChange={onChange}>
            ...
            <Select id="medicine" options={options4} filterOption={filterOption} onChange={handleChange4} name="medicine" placeholder="Select Medicine" />

        </div>
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="dosage" className="form-label">Enter Dosage:</label>
            <input type="text" className="form-control" id="dosage" name="dosage" onChange={onChange} />
      </div>
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="frequency" className="form-label">Enter Frequency:</label>
            <input type="text" className="form-control" id="frequency" name="frequency" onChange={onChange} />
      </div>
    ...
    <div className='mx-0' style={{display:'flex'}}>
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            <input type="text" className="form-control" id="duration" name="duration" onChange={onChange} />
      </div>
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="instructions" className="form-label">Enter Instructions:</label>
            <textarea className="form-control" id="instructions" name="instructions" onChange={onChange} />
      </div>
     <div className="mb-3 ms-3" style={{width:'100%'}}>
          <label htmlFor="abc" className="form-label" style={{display:'none'}}>abc</label>
          <input type="text" className="form-control" style={{display:'none'}} id="abc" name="abc"/>
      ...
      <button disabled={issuedDate==undefined||credentials.dosage==""||credentials.frequency==""||credentials.notes==""||credentials.instructions==""||credentials.duration==""} type="submit" className="btn btn-primary">Add Prescription</button>
      </form>
    </div>
  )
}
