...
const AdminLabTest = () => {
  ...
  const handleEdit = (id) => {
    ...
    navigate('editlabtest', { state: { labtest:dataitem} });
  };
  ...
  useEffect(() => {
      ...
      fetchData();
       ...
       useEffect(() => {
                  ...
                  console.log(labtests.length)
                  if(labtests.length>200&&labtests.length<401){
                      setEntries(25)
                    }
                    else  if(labtests.length>400&&labtests.length<701){
                      setEntries(50)
                    }
                     else  if(labtests.length>700){
                      setEntries(100)
                    }
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4 mobile-margin" onClick={handleClick}>Add Lab Test</button>
