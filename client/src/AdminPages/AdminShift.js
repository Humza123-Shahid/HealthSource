...
const AdminShift = () => {
  ...
  useEffect(() => {
      ...
      fetchData();
       ...
       useEffect(() => {
            ...
            if(shifts.length>200&&shifts.length<401){
                setEntries(25)
              }
              else  if(shifts.length>400&&shifts.length<701){
                setEntries(50)
              }
               else  if(shifts.length>700){
                setEntries(100)
              }
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4 mobile-margin" onClick={handleClick}>Add Shift</button>
