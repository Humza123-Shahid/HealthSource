...
const AdminRole = () => {
    ...
    const {roles,deleteRole,getRoles}=context;
  ...
  //   }
  // }, []);
  const filteredData = Array.isArray(roles) && roles.filter(item =>
      item.name?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  const handleView = (id,index) => {
    const dataitem=roles.find(da => da._id ==id)
    navigate('getrole', { state: { role:dataitem,idx:index} });
     
  };
  const handleEdit = (id) => {
    const dataitem=roles.find(da => da._id ==id)
    navigate('editrole', { state: { role:dataitem} });
  };
