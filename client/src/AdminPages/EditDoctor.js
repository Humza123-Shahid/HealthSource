...
const EditDoctor = () => {
  ...
  const editDoctors=async (e)=>{
          e.preventDefault();
          const success= await editDoctor(Doctor._id,selectedStaffValue,specializations,licenseNumber,experienceYears,consultationFee,selectedonCallValue,signatureUrl)
          console.log(success);
          if(success)
          {
            setShowToast(true);
  ...
  return (
    ...
    <div className="mb-3 ms-3" style={{width:'100%'}}>
     ...
     <div className="mb-3 ms-3" style={{width:'100%'}}>
      ...
      <button disabled={specializations.length<1||licenseNumber.length<1||experienceYears.length<1||consultationFee.length<1||signatureUrl.length<1||selectedStaffValue==''} type="submit" className="btn btn-primary" >Edit Doctor</button>
      </form>
    </div>
  )
}
