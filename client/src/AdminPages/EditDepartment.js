...
const filterOption = (option, inputValue) => {
  ...
  const editDepartments=async (e)=>{
          e.preventDefault();
          const success= await editDepartment(Department._id,name,code,description)
          console.log(success);
          if(success)
          {
            setShowToast(true);
  ...
  return (
    ...
    <div className="mb-3 ms-3" style={{width:'100%'}}>
     ...
     <div className="mb-3 ms-3" style={{width:'100%'}}>
      ...
      <button disabled={name.length<1||code==''||description==''} type="submit" className="btn btn-primary" >Update Department</button>
      </form>
    </div>
  )
}
