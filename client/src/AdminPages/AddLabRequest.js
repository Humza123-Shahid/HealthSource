...
const AddLabRequest = () => {
   const [showToast,setShowToast]=useState(false)
     ...
     const [credentials,setCredentials] =useState({patientId:null,doctorId:null,labtestId:null,priority:"routine",status:"pending",notes:""})
         const [ requestDate, setRequestDate] = useState(undefined);
         const [ requestDate2, setRequestDate2] = useState(undefined);
...
const filterOption = (option, inputValue) => {
  ...
  const addLabRequests=async (e)=>{
         e.preventDefault();
        const {patientId,doctorId,labtestId,priority,status,notes}=credentials

         
          const user=await addLabRequest(patientId,doctorId,labtestId,requestDate2,priority,status,notes)
          console.log(user)
          if(user.success)
          {
            ...
            console.log("abc");
            setShowToast(true);
            setMsg("Lab Request added successfully")
            setType("success")
            setTimeout(()=>{
              setShowToast(false)
            },1500)
  ...
  return (
    ...
    <div className='mx-0' style={{display:'flex'}}>
        ...
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            {/* <select id="labtestId" className="form-control " name="labtestId" onChange={onChange}>
             ...
             <Select id="labtestId" options={options3} filterOption={filterOption} onChange={handleChange3} name="labtestId" placeholder="Select Lab Test" />
        </div>
      ...
      <div className='mx-0' style={{display:'flex'}}>
       ...
       <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            {/* <input type="text" className="form-control" id="priority" name="priority" onChange={onChange} /> */}
    ...
    <div className="mb-3 ms-3" style={{width:'100%'}}>
     ...
     <div className="mb-3 ms-3" style={{width:'100%'}}>
      ...
      <button disabled={requestDate==undefined||credentials.priority==""||credentials.status==""||credentials.notes==""} type="submit" className="btn btn-primary">Add Lab Request</button>
      </form>
    </div>
  )
}
