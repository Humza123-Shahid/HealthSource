...
const AddNurse = () => {
    ...
    const [selectedAssignedWardValue, setSelectedAssignedWardValue] = useState('');
...
const filterOption = (option, inputValue) => {
  ...
  const addNurses=async (e)=>{
          e.preventDefault();
          const success= await addNurse(selectedStaffValue,qualification,licenseNumber,selectedAssignedWardValue)
          console.log(success);
          if(success)
          {
            setShowToast(true);
     ...
     <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
      ...
      <div className='mx-0' style={{display:'flex'}}>
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            <Select id="wardId" options={options2} filterOption={filterOption} onChange={handleChange2} name="wardId" placeholder="Select Assigned Ward" />      
            </div>
      ...
      <button disabled={licenseNumber.length<1||qualification.length<1||selectedStaffValue==''||selectedAssignedWardValue==''} type="submit" className="btn btn-primary" >Add Nurse</button>
      </form>
    </div>
  )
}
