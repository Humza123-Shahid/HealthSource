...
const EditAdmission = () => {
   const [showToast,setShowToast]=useState(false)
    ...
    const ward=location.state?.wardId || {};
    const room=location.state?.roomId || {};
    const bed=location.state?.bedId || {};
    ...
    const patient=location.state?.patientId || null;
     ...
     const [credentials,setCredentials] =useState({patientId:patient,doctorId:doctor,wardId:ward,roomId:room,bedId:bed,reason:Admission.reason,conditionOnAdmission:Admission.conditionOnAdmission,status:Admission.status})
         const [ admissionDate, setAdmissionDate] = useState(formatDate(new Date(Admission.admissionDate)));
         const [ admissionDate2, setAdmissionDate2] = useState(formatDate(new Date(Admission.admissionDate)));
            const [ dischargeDate, setDischargeDate] = useState(formatDate(new Date(Admission.dischargeDate)));
         const [ dischargeDate2, setDischargeDate2] = useState(formatDate(new Date(Admission.dischargeDate)));

    const context=useContext(admissionContext);
    const {editAdmission}=context;
...
const defaultValue3 = options3.find(d=>d.value==ward);
const defaultValue4 = options4.find(d=>d.value==room);
const defaultValue5 = options5.find(d=>d.value==bed);
...
const filterOption = (option, inputValue) => {
  ...
  const editAdmissions=async (e)=>{
         e.preventDefault();
        const {patientId,doctorId,wardId,roomId,bedId,reason,conditionOnAdmission,status}=credentials

         
          const user=await editAdmission(Admission._id,patientId,doctorId,wardId,roomId,bedId,admissionDate2,dischargeDate2,reason,conditionOnAdmission,status)
          console.log(user)
          if(user.success)
          {
            ...
            console.log("abc");
            setShowToast(true);
            setMsg("Admission updated successfully")
            setType("success")
            setTimeout(()=>{
              setShowToast(false)
            },1500)
    ...
    <div className='mx-0' style={{display:'flex'}}>
        ...
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            {/* <select id="wardId" className="form-control " name="wardId" onChange={onChange}>
             ...
             <Select id="wardId" options={options3} filterOption={filterOption} defaultValue={defaultValue3} onChange={handleChange3} name="wardId" placeholder="Select Ward" />
        </div>
      ...
      <div className='mx-0' style={{display:'flex'}}>
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="roomId" className="form-label">Room</label>
             <Select id="roomId" options={options4} filterOption={filterOption} defaultValue={defaultValue4} onChange={handleChange4} name="roomId" placeholder="Select Room" />
        </div>
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="bedId" className="form-label">Bed</label>
             <Select id="bedId" options={options5} filterOption={filterOption} defaultValue={defaultValue5} onChange={handleChange5} name="bedId" placeholder="Select Bed" />
        </div>
        <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
     ...
     <div className='mx-0' style={{display:'flex'}}>
      ...
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            ...
            <textarea className="form-control" id="reason"value={credentials.reason} name="reason" onChange={onChange} />
      </div>
      <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
            <label htmlFor="conditionOnAdmission" className="form-label">Enter Condition On Admission:</label>
            <textarea className="form-control" id="conditionOnAdmission" value={credentials.conditionOnAdmission} name="conditionOnAdmission" onChange={onChange} />
      </div>
        </div>
     ...
     <div className="mb-3 ms-3" style={{width:'100%'}}>
      ...
      <button disabled={dischargeDate==undefined||credentials.reason==""||credentials.status==""||credentials.conditionOnAdmission==""} type="submit" className="btn btn-primary">Update Admission</button>
      </form>
    </div>
  )
}
