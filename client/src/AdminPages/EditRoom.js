  ...
  const location = useLocation();
    const Room=location.state?.room || {};
     const Wards=location.state?.ward || {};
        const [chargesPerDay, setChargesPerDay] = useState(Room.chargesPerDay);
    const [roomType, setRoomType] = useState(Room.type);
    const [roomNumber, setRoomNumber] = useState(Room.roomNumber);
    const [selectedWardValue, setSelectedWardValue] = useState(Wards._id);
...
const defaultValue = options.find(d=>d.value==Wards._id);

const filterOption = (option, inputValue) => {
  // Only filter based on the 'label' property, for example
  return option.label.toLowerCase().includes(inputValue.toLowerCase());
};
  const editRooms=async (e)=>{
          e.preventDefault();
          const success= await editRoom(Room._id,selectedWardValue,roomNumber,roomType,chargesPerDay)
          console.log(success);
          if(success)
          {
            setShowToast(true);
...
useEffect(() => {
  ...
  return (
    ...
    <form onSubmit={editRooms}>

    <div className='mx-0' style={{display:'flex'}}>
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
        <label htmlFor="mySelect" className="form-label">Select Ward:</label>
        <Select id="wardId" options={options} filterOption={filterOption} defaultValue={defaultValue} onChange={handleChange} name="wardId" placeholder="Select Ward" />      
    </div>
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
    ...
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
      ...
      <button disabled={roomNumber.length<1||roomType.length<1||selectedWardValue==''} type="submit" className="btn btn-primary" >Update Room</button>
      </form>
    </div>
  )
}
