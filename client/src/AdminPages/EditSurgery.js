...
const EditSurgery = () => {
  ...
  const editSurgeries=async (e)=>{
        ...
        //  setArrivalTime("1970-09-03T"+arrivalTime);
          const success=await editSurgery(Surgery._id,selectedPatientValue,selectedSurgeonValue,surgeryType,scheduledDate2,startTime2,endTime2,selectedTheatreValue,notes)
          console.log(success)
          if(success)
          {
            console.log("abc");
...
useEffect(() => {
  ...
  return (
    ...
    <div className='mx-0' style={{display:'flex'}}>
      ...
      <button disabled={startTime.length<1||endTime.length<1||scheduledDate.length<1||selectedPatientValue==''||selectedSurgeonValue==''||surgeryType==''} type="submit" className="btn btn-primary">Edit Surgery</button>
      </form>
    </div> 
  )
}
