...
const AdminRoom = () => {
  ...
  useEffect(() => {
      ...
      fetchData();
       ...
       useEffect(() => {
            ...
            if(rooms.length>200&&rooms.length<401){
                setEntries(25)
              }
              else  if(rooms.length>400&&rooms.length<701){
                setEntries(50)
              }
               else  if(rooms.length>700){
                setEntries(100)
              }
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4 mobile-margin" onClick={handleClick}>Add Room</button>
